...
const uncheckedBikesRouter = express.Router();
...
uncheckedBikesRouter.post('/api/bikes/unchecked', tokenVerifier, tokenAuthCheck, function (req, res) {
    try {
        jwt.verify(req.token, process.env.key, function (error, authData) {
            if (error) {
                ...
                if (!authData['privilages'].includes('bikes')) {
                    ...
                    try {
                        bikeDAOForRetrieval.getAllUnchecked(function (status, message, data) {
...
module.exports = uncheckedBikesRouter;
