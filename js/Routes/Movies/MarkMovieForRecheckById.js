...
const MarkMovieForRecheckRouter = express.Router();
...
MarkMovieForRecheckRouter.post('/api/movie/mark_recheck', tokenVerifier, tokenAuthCheck, movieIdVerifier, function (req, res) {
    try {
        jwt.verify(req.token, process.env.key, function (error, authData) {
            if (error) {
                ...
                if (!authData['privilages'].includes('movies')) {
                    ...
                    try {
                        var id = req.body._id;
                        movieDAO.markForRecheckById(id, function (status, message, data) {
...
module.exports = MarkMovieForRecheckRouter;
