...
const ApproveCarRouter = express.Router();
...
ApproveCarRouter.post('/api/car/approve', tokenVerifier, tokenAuthCheck, carIdVerifier, function (req, res) {
    try {
        jwt.verify(req.token, process.env.key, function (error, authData) {
            if (error) {
                ...
                if (!authData['privilages'].includes('cars')) {
                    ...
                    try {
                        var id = req.body._id;
                        carDAO.approveById(id, function (status, message, data) {
                            if (status === 200) {
...
module.exports = ApproveCarRouter;
