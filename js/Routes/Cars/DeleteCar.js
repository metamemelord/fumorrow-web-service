...
const carRequestVerifier = require('../RouteUtils').requestIdVerifier;
...
deleteCarRouter.post('/api/car/delete', tokenVerifier, tokenAuthCheck, carRequestVerifier, function (req, res) {
    try {
        jwt.verify(req.token, process.env.key, function (error, authData) {
            if (error) {
                ...
                if (!authData['privilages'].includes('cars')) {
                    ...
                    try {
                        var id = req.body._id;
                        carDAO.removeById(id, function (status, message, data) {
