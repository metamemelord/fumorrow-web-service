...
addVideoGameRouter.post(
  "/api/videogame/add",
  tokenVerifier,
  tokenAuthCheck,
  videoGameRequestVerifier,
  function(req, res) {
    try {
      jwt.verify(req.token, process.env.key, function(error, authData) {
        if (error) {
          ...
          if (!authData["privilages"].includes("video_games")) {
            ...
            var videoGameObject = {
              _id:
                videoGameData.day.toString() +
                (videoGameData.month - 1).toString() +
                videoGameData.year.toString(),
            ...
            length = 12 - videoGameObject._id.length;
            videoGameObject._id += helpers.generateSalt(length);
            var uniqueId =
              videoGameObject.title +
              videoGameObject.release_date.toString() +
              videoGameData.referrerName;
            uniqueId = uniqueId.replace(/\s/g, "");
            videoGameObject.uid = md5(uniqueId);
            videoGameObject.genres.sort();
            videoGameObject.is_released = helpers.checkDate(
              videoGameObject.release_date
            );
            videoGameDAO.addVideoGame(videoGameObject, function(
              status,
              message,
              data
            ) {
