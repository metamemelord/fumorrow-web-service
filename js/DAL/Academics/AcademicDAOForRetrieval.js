...
let academicDBService = connectionForRetrieval.model("academics", academicSchema);


function getAll(callback) {
	academicDBService.find({ "is_approved": true }, privateAcademicFields).sort({ "_id": 1 }).exec(function (error, data) {
		if (error) {
			logger.error(error);
...
function getInRange(begin, limit, callback) {
	academicDBService.find({ "is_approved": true }, privateAcademicFields).sort({ "_id": 1 }).skip(begin).limit(limit).exec(function (error, data) {
		if (error) {
			logger.error(error);
			callback(500, "Internal server error", null);
		}
...
function getAllByFilter(filter, callback) {
	academicDBService.find({
		$and: [
			{ $or: [{ "language": { "$in": filter } }, { "genres": { "$in": filter } }] },
			{ "is_approved": true }
		]
	}, privateAcademicFields).sort({ "_id": 1 }).exec(function (error, data) {
		if (error) {
			logger.error(error);
			callback(500, "Internal server error", null);
		}
...
function getInRangeByFilter(filter, begin, limit, callback) {
	...
	}, privateAcademicFields).sort({ "_id": 1 }).skip(begin).limit(limit).exec(function (error, data) {
		if (error) {
			logger.error(error);
			callback(500, "Internal server error", null);
		}
